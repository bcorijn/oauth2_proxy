...
func (p *GoogleProvider) RefreshSessionIfNeeded(s *SessionState) (bool, error) {
	...
	newToken, duration, newIdToken, err := p.redeemRefreshToken(s.RefreshToken)
	if err != nil {
		return false, err
	}
...
func (p *GoogleProvider) redeemRefreshToken(refreshToken string) (accessToken string, expires time.Duration, idToken string, err error) {
